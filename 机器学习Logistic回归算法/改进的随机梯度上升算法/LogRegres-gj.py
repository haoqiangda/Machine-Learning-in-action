...
def gradAscent(dataMatIn, classLabels):
	...
	return weights.getA(),weights_array									#将矩阵转换为数组，并返回,weights_array - 每次更新的回归系数
"""
+函数说明:改进的随机梯度上升算法
+
+Parameters:
...
def stocGradAscent1(dataMatrix, classLabels, numIter=150):
	...
	dataIndex = range(m)										#返回dataMatrix的大小。m为行数,n为列数。
	...
	for j in range(numIter):											
		for i in range(m):			
			...
			del(list(dataIndex)[randIndex])										#删除已经使用的样本
