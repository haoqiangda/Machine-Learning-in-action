...
def colicTest():
	...
	#print trainWeights
	#trainWeights1= gradAscent(np.array(trainingSet), trainingLabels)                   										#使用jibende 上升梯度训练
...
def colicTest1():
	...
	errorRate = (float(errorCount)/numTestVec) * 100 
	#errorRate1= (float(errorCount1)/numTestVec) * 100 								#错误率计算
	print("jiben测试集错误率为: %.2f%%" % errorRate)
	return errorRate
...
def colicSklearn():
	...
	for line in frTrain.readlines():
		...
		for i in range(len(currLine)-1):
			lineArr.append(float(currLine[i]))
			lineArr1.append(float(currLine[-1]))
		trainingSet.append(lineArr)
		trainingLabels.append(lineArr1)
	for line in frTest.readlines():
		currLine = line.strip().split('\t')
		lineArr =[]
		for i in range(len(currLine)-1):
